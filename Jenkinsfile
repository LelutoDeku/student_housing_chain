        ...
        stage('Push to ECR') {
            steps {
                script {
                    ...
                    sh "docker tag $DOCKER_IMAGE_NAME_FOR_CLIENT:latest ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_DEFAULT_REGION}.amazonaws.com/${ECR_REPO_NAME}:latest"
                    sh "docker tag $DOCKER_IMAGE_NAME_FOR_SERVER:latest ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_DEFAULT_REGION}.amazonaws.com/${ECR_REPO_NAME}:latest"
